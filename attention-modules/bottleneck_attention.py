"""
Bottleneck Transformers for Vision Recognition
This paper just used Multi-Head Self-Attention module
replace 3*3 Conv at ResNet50 last block.
...
class BottleBlock(nn.Module):
    def __init__(self, *, dim, fmap_size, dim_out, proj_factor,
                 downsample, heads=4, dim_head=128,
                 rel_pos_emb=False, activation=nn.ReLU()):
        super().__init__()
...
class BottleStack(nn.Module):
    def __init__(self, *, dim, fmap_size, dim_out=2048,
                 proj_factor=4, num_layers=3, heads=4,
                 dim_head=128, downsample=True,
                 rel_pos_emb=False, activation=nn.ReLU()):
        super().__init__()
